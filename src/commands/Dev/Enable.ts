...
export default class Command extends BaseCommand {
    ...
    run = async (M: ISimplifiedMessage, { joined }: IParsedArgs): Promise<void> => {
        ...
        if (!key) return void (await M.reply(`My Darling please provide the command you want to enable for everyone`))
        ...
        await M.reply(`*${this.client.util.capitalize(command.config.command)}* is now Enabled successfully`)
    }
}
